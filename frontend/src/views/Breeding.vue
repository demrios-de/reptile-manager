        ...
        <div class="relative bg-surface-700 rounded-2xl border border-surface-500 w-full max-w-md shadow-2xl p-6">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div><label>Geburtsdatum</label><input type="date" v-model="bulkDob" /></div>
              <div>
            ...
            <div class="flex gap-3">
              <button class="btn-primary flex-1" :disabled="savingBulk" @click="createBulk">
                {{ savingBulk ? 'Anlegen…' : `${bulkQty} Tier(e) anlegen` }}
              </button>
              <button class="btn-secondary" @click="showBulkModal = false">Abbrechen</button>
            </div>
          </div>
        </div>
      </div>
